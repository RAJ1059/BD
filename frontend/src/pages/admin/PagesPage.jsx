...
export default function PagesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await pagesApi.list({ page, limit: 20, search: debouncedSearch || undefined })
      setRows(res.data)
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      await pagesApi.remove(deleteTarget._id)
    ...
    { key: 'publishedAt', header: 'Published', render: (row) => (row.publishedAt ? new Date(row.publishedAt).toLocaleDateString() : '—') },
    {
      ...
      render: (row) => (
        <div className="flex justify-end gap-2">
          <button
            onClick={() => navigate(`/admin/pages/${row._id}`)}
  ...
  return (
    <div>
      <PageHeader
        title="Pages"
        description="Manage static CMS pages such as About and Services."
        actions={
          <PrimaryButton onClick={() => navigate('/admin/pages/new')}>
            <FiPlus /> New page
          </PrimaryButton>
        }
      />
