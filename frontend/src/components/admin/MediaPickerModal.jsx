  ...
  return (
    <Modal open={open} onClose={onClose} title="Select media" width="max-w-3xl">
      ...
      {loading ? (
        ...
        <p className="text-[#6B6B78]">No media uploaded yet. Upload files from the Media Library first.</p>
      ) : (
        <div className="grid max-h-96 grid-cols-3 gap-3 overflow-y-auto sm:grid-cols-4">
          {rows.map((item) => (
            <button
              ...
              className="overflow-hidden rounded-xl border border-white/10 bg-[#09090B] text-left transition hover:border-[#A050F8]"
            >
              <div className="flex aspect-square items-center justify-center">
                {item.type === 'image' ? (
                  <img src={item.thumbnailUrl || item.url} alt={item.originalName} className="h-full w-full object-cover" />
                ) : (
                  <FiFile size={24} className="text-[#6B6B78]" />
                )}
              </div>
