...
export default function PageHero({ eyebrow, title, description, children, graphic }) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#0B0E14] via-[#141928] to-[#0d1f3a] py-24 text-white">
      ...
      <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at bottom left, #05B0BA 0%, transparent 45%)' }} />
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#22D3D9]">{eyebrow}</p>
