    ...
    <div className="bg-[#0B0E14]">
      ...
      {error && (
        <section className="mx-auto max-w-7xl px-4 pt-16 sm:px-6 lg:px-8">
          <p className="rounded-2xl border border-[#22D3D9]/30 bg-[#22D3D9]/10 px-5 py-4 text-sm text-[#22D3D9]">{error}</p>
        </section>
      )}

      {!error && !loading && posts.length === 0 && (
        <section className="mx-auto max-w-7xl px-4 pt-16 sm:px-6 lg:px-8">
          <div className="rounded-[32px] border border-white/10 bg-[#141928] p-10 text-center text-[#8B93A7]">
      ...
      {featured && (
        <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <motion.article
            ...
            viewport={{ once: true }}
            className="group grid overflow-hidden rounded-[32px] border border-white/10 bg-[#141928] shadow-sm transition hover:shadow-xl lg:grid-cols-2"
          >
            <div className="relative h-64 overflow-hidden lg:h-full">
              ...
              />
              <span className="absolute left-5 top-5 rounded-full bg-[#22D3D9] px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-white">Featured</span>
            </div>
            <div className="flex flex-col justify-center p-8 lg:p-12">
              <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#05B0BA]">{featured.category?.name || 'General'}</p>
              <h2 className="mt-4 text-2xl font-semibold text-white sm:text-3xl">{featured.title}</h2>
              <p className="mt-4 text-sm leading-7 text-[#8B93A7]">{featured.excerpt}</p>
              <div className="mt-6 flex items-center gap-4 text-xs text-[#5B6478]">
              ...
              <Link to={`/blog/${featured.slug}`} className="mt-8 inline-flex w-fit items-center gap-2 text-sm font-semibold text-[#05B0BA]">
            ...
            return (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                  activeCategory === slug ? 'bg-[#05B0BA] text-white' : 'bg-white/5 text-[#8B93A7] hover:bg-white/10'
                }`}
              >
                {label}
              </button>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {loading &&
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="h-[340px] animate-pulse rounded-[28px] border border-white/10 bg-[#141928]" />
            ))}
          ...
          {!loading &&
            rest.map((post, index) => (
              <motion.article
                ...
                className="group overflow-hidden rounded-[28px] border border-white/10 bg-[#141928] shadow-sm transition hover:-translate-y-1 hover:shadow-xl"
              >
                ...
                <div className="p-6">
                  ...
                  <p className="mt-3 text-sm leading-7 text-[#8B93A7]">{post.excerpt}</p>
                  <div className="mt-5 flex items-center gap-3 text-xs text-[#5B6478]">
                  ...
                  <Link to={`/blog/${post.slug}`} className="mt-5 inline-flex text-sm font-semibold text-[#05B0BA]">Read more →</Link>
                </div>
              </motion.article>
            ))}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-6 rounded-[36px] border border-white/10 bg-[#141928] p-10 text-center shadow-sm">
          ...
          <p className="max-w-xl text-sm leading-7 text-[#8B93A7]">Get our best growth thinking delivered straight to your inbox, once a month.</p>
          <form className="flex w-full max-w-md flex-col gap-3 sm:flex-row" onSubmit={(e) => e.preventDefault()}>
            <input type="email" required placeholder="Your work email" className="w-full rounded-full border border-white/10 bg-white/5 px-5 py-3 text-sm text-white outline-none placeholder:text-[#5B6478] focus:border-[#05B0BA]" />
